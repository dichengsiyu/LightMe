...
public class MLisenseMangaer {
	...
	public void bindRemoteService() {
		if(mLicensingService == null) {
			...
			mContext.bindService(serviceIntent, mLicenseConnection, Context.BIND_AUTO_CREATE);
		} else {
			if(mListener != null)
				mListener.onRemoteServiceConnected();
		}
	...
	public int doRemoteCheck() {
		...
		if (result != null 
			...
			boolean bSuccess = LicenseCheckHelper.checkResult(APK_PUBLIC_KEY,
					result);
			...
			if (bSuccess
				...
				//FIXME 获取子产品
					...
					//Log.v("", "试用开始的日期为: " + beginCal.get(Calendar.YEAR) + "年"
